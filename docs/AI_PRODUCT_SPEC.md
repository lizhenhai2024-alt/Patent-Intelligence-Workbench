...
## 11. 受控智能体约定（2026-09-23 决定）

AI 智能体只能在以下限制内自行决定下一步：

- 工具由白名单限定，且均为只读，除非以后的 SPEC 明确增加受控的写入；每次运行有步数上限。
- 智能体输出中的每条结论都必须引用存在于本地库或某个已标明数据源响应中的公开号；由代码而不是模型拒绝或标记没有引用的结论。
- 智能体提出的检索词，在对远程数据源执行前先展示给用户确认。
- 智能体只产出候选和证据。X/Y/A 分类、新颖性、FTO、侵权和风险判断始终由人做出，绝不作为自动结论或概率评分输出。
- 模型由用户从已配置的云端模型中选择，不假定本地部署。发送给模型的内容必须事先向用户说明（智能体页在运行按钮旁常驻显示发送对象和内容范围；产品负责人 2026-09-23 决定取消逐次弹窗）；用户私有字段（笔记、项目、标签、监控规则、未公开的设计资料）默认排除，除非用户在该次请求中主动提供。
