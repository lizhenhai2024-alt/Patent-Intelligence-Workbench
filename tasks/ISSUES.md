...
先修 UI-01 错误归属；再处理 UI-02/04/05/06/07/08/09/10 的状态与数据范围，UI-11/12 的受控任务边界以及 UI-13 的入口可达性；UI-03 和其他 P1 同批验证，最后处理 P2。环境 ENV-01 单独排查。每项修复应增加针对其复现条件的回归测试，不能以本轮已有测试通过代替缺陷验证。
